...
const VERSION = '5.3.3-ev-cushion-integrity';
...
async function fetchWithTimeout(url,options={},timeout=9000){ const c=new AbortController(); const t=setTimeout(()=>c.abort(),timeout); try{return await fetch(url,{...options,signal:c.signal,headers:{'User-Agent':'SB101-AEGIS-Zero-Cost/5.3.3','Accept':'application/json,text/plain,*/*',...(options.headers||{})}});}finally{clearTimeout(t);} }
...
server.listen(PORT,'0.0.0.0',()=>console.log(`AEGIS Command UX v5.3.3 running on ${PORT} • ${MODELS.length} models • OpenAI calls disabled`));
