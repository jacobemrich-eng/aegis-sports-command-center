...
const VERSION = '5.0.0-results-lab';
...
async function fetchWithTimeout(url,options={},timeout=9000){ const c=new AbortController(); const t=setTimeout(()=>c.abort(),timeout); try{return await fetch(url,{...options,signal:c.signal,headers:{'User-Agent':'SB101-AEGIS-Zero-Cost/4.0','Accept':'application/json,text/plain,*/*',...(options.headers||{})}});}finally{clearTimeout(t);} }
...
const HTML = `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#06131d"><title>SB101 AEGIS Results Lab</title><style>
...
</style></head><body><div class="wrap"><header class="hero"><div class="eyebrow">SB101 AEGIS v5 • RESULTS & CALIBRATION LAB</div><h1>Sports Command Center</h1><p class="muted">One tap: live odds → free structured sports data → blind projection → market challenger → release gates → disciplined final card.</p><div class="status" id="status">Checking free data engine…</div></header>
<nav class="tabs"><button class="tab active" data-tab="dashboard">Dashboard</button><button class="tab" data-tab="card">Final Card</button><button class="tab" data-tab="results">Results Lab</button><button class="tab" data-tab="models">Model Registry</button></nav>
<section id="dashboard"><div class="panel"><div class="controls"><div class="control"><label>Sport</label><select id="sport"></select></div><div class="control"><label>Markets</label><select id="markets"><option value="h2h,spreads,totals">ML + Spread + Total</option><option value="h2h">Moneyline only</option><option value="spreads">Spread only</option><option value="totals">Total only</option></select></div></div><button class="btn secondary" id="sync">Sync Pregame Board</button><button class="btn primary" id="scan">RUN FULL AUTOMATIC SLATE SCAN</button><p class="muted small">No model sliders. No OpenAI billing. Pregame only: already-started games are removed before analysis. The main scan button automatically syncs a fresh pregame board first when needed. The engine lowers Data Quality or PASSes when free feeds cannot verify a key input.</p><div id="quota" class="muted small"></div><div id="enhance" class="notice small"></div></div>
...
function exportAudit(){const blob=new Blob([JSON.stringify({exported_at:new Date().toISOString(),version:'5',records:loadAudit()},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='aegis-results-backup-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
...
async function init(){const h=await api('/api/health');$('#status').textContent=(h.odds_ready?'ODDS READY':'ODDS KEY NEEDED')+' • ZERO-COST RESULTS LAB READY • '+h.models+' models';$('#enhance').innerHTML='<b>Free-data status:</b> MLB public Stats API + ESPN structured feeds + Open-Meteo + nflverse are automatic. '+(h.cfbd_ready?'<b>NCAAF advanced CFBD ratings active.</b>':'NCAAF works in basic mode. Optional free CFBD key can raise NCAAF data quality; no credit card required.');const s=await api('/api/sports');$('#sport').innerHTML=s.sports.map(x=>'<option value="'+esc(x.key)+'">'+esc(x.title)+'</option>').join('');const m=await api('/api/models');$('#modelGrid').innerHTML=m.models.map(x=>'<div class="model"><b>'+esc(x[0])+'</b><div class="muted small">'+esc(x[1])+'</div><p>'+esc(x[2])+'</p></div>').join('');const resetBoard=()=>{EVENTS=[];LAST_SYNC=0;$('#events').innerHTML='';};$('#sport').onchange=resetBoard;$('#markets').onchange=resetBoard;$('#gradeResults').onclick=()=>gradeAudit(false);$('#exportResults').onclick=exportAudit;$('#clearResults').onclick=()=>{if(confirm('Clear AEGIS results stored on this device?')){localStorage.removeItem(AUDIT_KEY);renderAudit()}};renderAudit();setTimeout(autoGrade,800)}
...
function renderCard(d){let h='<div class="muted small">SLATE GRADE</div><h2>'+esc(d.slate_grade||'—')+'</h2><p class="muted">Generated '+new Date(d.generated_at).toLocaleString()+' • calibrated deterministic/free-data engine</p>';
if(d.slate_sanity&&d.slate_sanity.triggered)h+='<div class="notice warning"><b>MARKET SANITY FIREWALL</b><p>'+esc(d.slate_sanity.message)+'</p></div>';
if(!(d.plays||[]).length)h+='<div class="notice"><b>NO QUALIFIED BETS</b><p class="muted">AEGIS did not release a bet. Missing coverage, large model/market disagreement, or weak market-specific evidence is treated as uncertainty rather than fake confidence.</p></div>';
for(const p of d.plays||[]){const pr=p.projection||{},range=pct(p.fair_probability_low)+' – '+pct(p.fair_probability_high);let flags=(p.sanity_flags||[]).map(x=>'<div class="notice warning small">'+esc(x)+'</div>').join('');h+='<div class="play"><span class="badge '+esc(p.tier)+'">'+esc(p.tier)+'</span><b>'+esc(p.event.away_team)+' @ '+esc(p.event.home_team)+'</b><h3>'+selectionLabel(p)+'</h3><div class="metricgrid">'+metric('Projected score',esc(pr.projected_score.away)+' – '+esc(pr.projected_score.home))+metric('Raw model',pct(p.fair_raw))+metric('Uncertainty-adjusted independent',pct(p.fair_independent))+metric('Calibrated fair',pct(p.fair_probability))+metric('Fair range',range)+metric('Market challenger',pct(p.market_probability))+metric('Independent-market gap',pct(p.independent_disagreement))+metric('Raw-model gap',pct(p.raw_market_gap))+metric('Band vs market',p.market_inside_fair_range?'OVERLAPS':'CLEARS')+metric('Adjusted edge',pct(p.adjusted_edge))+metric('Estimated EV',pct(p.estimated_ev))+metric('Data quality',Math.round(pr.data_quality)+'/100')+metric('Market raw agreement',Math.round(p.market_model_agreement==null?(pr.model_agreement||0):p.market_model_agreement)+'/100')+metric('Market coverage',Math.round(p.market_coverage==null?(pr.coverage_score||0):p.market_coverage)+'/100')+metric('Effective agreement',Math.round(p.market_effective_agreement==null?(pr.effective_agreement||0):p.market_effective_agreement)+'/100')+metric('Market-specific edges',esc(p.market_edge_count==null?pr.independent_edge_count:p.market_edge_count))+metric('Support strength',Math.round(p.market_support_strength||0)+'/100')+metric('Uncertainty',Math.round(pr.uncertainty||0)+'/100')+metric('Required cushion',pct(p.cushion))+metric('Target book',p.hard_rock?'VERIFIED':'NOT VERIFIED')+metric('Stake',p.units+'u')+(p.push_probability>0?metric('Push probability',pct(p.push_probability)):'')+'</div>'+flags+f5Html(p)+'<p><b>Why:</b> '+esc(p.why)+'</p><p><b>Timing:</b> '+esc(p.timing)+'</p><p class="risk"><b>How it loses / risk:</b> '+esc((p.how_it_loses||[]).join(' • '))+'</p><p class="muted"><b>Final verification:</b> '+esc(p.final_verification)+'</p><details><summary>Applied models & source evidence</summary>'+modulesHtml(p)+'<div class="hr"></div>'+((pr.notes||[]).map(n=>'<div class="small muted">• '+esc(n)+'</div>').join(''))+'<div class="hr"></div>'+((pr.sources||[]).map(s=>'<a class="source" target="_blank" rel="noopener" href="'+esc(s.url)+'">'+esc(s.name)+'</a>').join('')||'<span class="muted">No external source links available.</span>')+'</details></div>'}
if(d.parlay)h+='<div class="panel"><h3>Optional qualified parlay • '+d.parlay.units+'u • approx '+fmtA(d.parlay.approx_american)+'</h3>'+d.parlay.legs.map(l=>'<div>'+esc(l.selection)+fmtPoint(l.market,l.point,l.selection)+' '+fmtA(l.price)+' • '+esc(l.book)+'</div>').join('')+'<p>'+esc(d.parlay.rationale)+'</p></div>';
...
server.listen(PORT,'0.0.0.0',()=>console.log(`AEGIS Results & Calibration Lab v5 running on ${PORT} • ${MODELS.length} models • OpenAI calls disabled`));
