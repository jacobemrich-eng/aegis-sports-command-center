...
const VERSION = '5.3.4-mlb-confirmation-integrity';
...
function marketCandidate(event,proj,pair,market){
  ...
  if(event.sport_key==='baseball_mlb'){
    ...
    if(market==='totals'&&!proj.probable_starters_confirmed){gate='PASS';sanityFlags.push('MLB probable-starter gate: a full-game total with one or both probable starters unconfirmed is an automatic CUT.');}
...
server.listen(PORT,'0.0.0.0',()=>console.log(`AEGIS Command UX v5.3.4 running on ${PORT} • ${MODELS.length} models • OpenAI calls disabled`));
