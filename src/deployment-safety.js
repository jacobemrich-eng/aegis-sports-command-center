...
const NCAAF_STAGING_ENVIRONMENT = 'ncaaf-shadow-staging';
...
function identity(env = process.env) {
  ...
  const nflStaging = environment === STAGING_ENVIRONMENT;
  const ncaafStaging = environment === NCAAF_STAGING_ENVIRONMENT;
  const staging = nflStaging || ncaafStaging;
  return {
    environment,
    state_id: stateId,
    nfl_shadow_staging: nflStaging,
    ncaaf_shadow_staging: ncaafStaging,
    shadow_staging: staging,
    shadow_only: staging,
    ...
    ncaaf_sim_enabled: enabled(env, 'NCAAF_SIM_ENABLED', false),
    ncaaf_shadow_only: enabled(env, 'NCAAF_SIM_SHADOW_ONLY', true),
...
function stagingViolations(env = process.env) {
  ...
  if (!row.shadow_staging) return [];
  const violations = [];
  const requiredState = row.ncaaf_shadow_staging ? NCAAF_STAGING_ENVIRONMENT : STAGING_ENVIRONMENT;
  if (row.state_id !== requiredState) violations.push(`AEGIS_STATE_ID must be ${requiredState}`);
  ...
  if (row.nfl_shadow_staging && !row.nfl_sim_enabled) violations.push('NFL_SIM_ENABLED must be true');
  if (row.nfl_shadow_staging && !row.nfl_shadow_only) violations.push('NFL_SIM_SHADOW_ONLY must be true');
  if (row.ncaaf_shadow_staging && !row.ncaaf_sim_enabled) violations.push('NCAAF_SIM_ENABLED must be true');
  if (row.ncaaf_shadow_staging && !row.ncaaf_shadow_only) violations.push('NCAAF_SIM_SHADOW_ONLY must be true');
...
function assertStagingConfiguration(env = process.env) {
  ...
  if (violations.length) throw new Error(`Unsafe shadow staging configuration: ${violations.join('; ')}`);
  return identity(env);
}

module.exports = {
  STAGING_ENVIRONMENT,
  NCAAF_STAGING_ENVIRONMENT,
  PRODUCTION_STATE_ID,
  REAL_RELEASE_SPORTS,
  identity,
  stagingViolations,
