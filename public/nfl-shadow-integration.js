(function(){
  ...
  var NFL='americanfootball_nfl', cache={sport:null,games:[],loaded:false,error:null}, loading=null;
  ...
  function n(value,digits){var x=Number(value);return Number.isFinite(x)?x.toFixed(digits==null?1:digits):'—'}
  function p(value){var x=Number(value);return Number.isFinite(x)?(x*100).toFixed(1)+'%':'—'}
  ...
  function fetchShadow(){
    if(currentSport()!==NFL){cache={sport:currentSport(),games:[],loaded:true,error:null};return Promise.resolve(cache)}
    if(cache.loaded&&cache.sport===NFL)return Promise.resolve(cache);
    if(loading)return loading;
    loading=fetch('/api/shadow/games?sport='+encodeURIComponent(NFL),{cache:'no-store'})
      .then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.json()})
      .then(function(data){cache={sport:NFL,games:data.games||[],loaded:true,error:null,flags:data.flags||{}};return cache})
      .catch(function(error){cache={sport:NFL,games:[],loaded:true,error:error.message||String(error)};return cache})
  ...
  document.querySelector('#sport')?.addEventListener('change',function(){cache={sport:null,games:[],loaded:false,error:null};if(currentSport()===NFL)fetchShadow()});
})();
